    ...
    <Flex :column="true" justify="center" items="center" class="mt-6 sm:gap-y-6">
      ...
      <Flex :column="true" justify="center" items="center" gap-y="5"
        ...
        <div
          ...
          <button class="px-3 " type="submit" @click="show = !show" :class="(password === ``) ? `hidden` : `inline`" >
            <Icon :icon="(show) ? `mdi:eye-off` : `mdi:eye`" class="size-6  inline"/>
          </button>
        </div>

        <button type="button" :class="transition" class="px-3 py-0.5  bg-blue-500 text-white rounded-lg font-semibold hover:bg-blue-600 hover:scale-105 duration-300">Sign Up</button>
